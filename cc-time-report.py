...
Usage:
    cc-time-report                    # Summary: all projects, last 7 days
    cc-time-report today              # Today only
    cc-time-report week               # This week (Mon–Sun)
    cc-time-report month              # This month
    cc-time-report all                # All time
    cc-time-report project <name>     # Single project, all time
    cc-time-report active             # Currently running sessions
    cc-time-report raw                # Raw JSONL dump (for piping)
    cc-time-report orphans            # Sessions with start but no end
    cc-time-report csv                # Export as CSV
"""
...
def load_sessions() -> list[dict]:
    """Load all session records from the JSONL file."""
...
def print_project_table(projects: dict[str, dict], title: str = "Projects"):
    ...
    for name, data in sorted_projects:
        ...
        pct = (dur / total_all * 100) if total_all > 0 else 0
